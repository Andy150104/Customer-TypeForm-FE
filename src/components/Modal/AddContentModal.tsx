...
const FIELD_PLACEHOLDERS: Record<FieldType, string> = {
	[FieldType.Value0]: "Type your answer",
	[FieldType.Value1]: "42",
	[FieldType.Value2]: "name@company.com",
	[FieldType.Value3]: "+84 123 456 789",
	[FieldType.Value4]: "Select a date",
	[FieldType.Value5]: "Select a time",
	[FieldType.Value6]: "Pick date and time",
	[FieldType.Value7]: "Write your response here",
	[FieldType.Value8]: "Choose an option",
	[FieldType.Value9]: "Select one or more options",
	[FieldType.Value10]: "Select all that apply",
	[FieldType.Value11]: "Pick one option",
	[FieldType.Value12]: "Upload your file",
	[FieldType.Value13]: "Rate from 1 to 5",
	[FieldType.Value14]: "Slide to rate",
	[FieldType.Value15]: "Yes or No?",
};

const getPlaceholderForType = (type: FieldType) => FIELD_PLACEHOLDERS[type] ?? "Type your answer";

const normalizeOptionValue = (label: string | undefined, value: string | undefined, index: number) => {
...
export const AddContentModal: React.FC<AddContentModalProps> = ({
	...
	const [autoPlaceholder, setAutoPlaceholder] = useState<string>(getPlaceholderForType(FieldType.Value0));
	...
	const resetFormValues = useCallback(() => {
		const placeholder = getPlaceholderForType(FieldType.Value0);
		form.setFieldsValue({
			title: "",
			description: "",
			isRequired: true,
		...
		setAutoPlaceholder(placeholder);
	}, [form]);
	...
	const handleSubmit = async () => {
		try {
			...
			const propertiesPayload = autoPlaceholder ? { placeholder: autoPlaceholder } : undefined;
			const optionsPayload = needsOptions
				? (lockedOptions ?? values.options ?? createOptionPlaceholders()).map((option, index) => ({
	...
	return (
		...
		>
			<Form form={form} layout="vertical" initialValues={{ isRequired: true }} className="space-y-6">
				<div
					...
					<div className="flex flex-wrap items-center gap-4">
						<div>
							...
							<Typography.Text type="secondary">
								Choose a field pattern on the left, then fine-tune the content on the right.
							</Typography.Text>
							{!formId && (
								<div className="mt-1 text-amber-500 text-sm">
									Form identifier is not ready yet. Please wait for the page to finish loading.
								</div>
							)}
						</div>
						<div
					...
					>
						<div className="flex items-center justify-between gap-3">
							<div>
								...
								<p className={`m-0 text-sm ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>
						...
						{FIELD_TYPE_SECTIONS.map((section) => {
							const items = FIELD_TYPES.filter((item) => item.category === section.key);
												...
												>
													<span
														className={`mt-1 text-lg ${
															isActive ? "text-indigo-400" : isDarkMode ? "text-slate-400" : "text-slate-500"
														}`}
													>
														{item.icon}
													</span>
					...
					<div className="space-y-4">
						...
						>
							<div className="flex items-center justify-between gap-3">
								...
								<Tag className="text-xs" color={isDarkMode ? "#94a3b8" : "#cbd5f5"}>
							...
							<Form.Item label="Answer placeholder" extra="Auto generated based on the selected field type">
								...
								<div className="rounded-2xl border border-dashed px-4 py-4">
									<div className="flex items-center justify-between gap-3">
										<div>
											...
											<p className={`m-0 text-xs ${isDarkMode ? "text-slate-400" : "text-slate-500"}`}>
									...
									{lockedOptions ? (
										...
										<Form.List
											...
											rules={[
												{
													validator: async (_: unknown, options?: OptionFormValue[]) => {
														if (!options || options.length < MIN_OPTION_COUNT) {
															return Promise.reject(new Error("Add at least two options."));
														}
														if (options.some((option) => !option?.label?.trim())) {
															return Promise.reject(new Error("Option labels cannot be empty."));
														}
												...
												<div className="mt-3 space-y-3">
													{fields.map((field, index) => (
														<Space key={field.key} className="flex w-full" align="baseline">
															...
															>
																<Input placeholder={`Option ${index + 1} label`} disabled={isSubmitting} />
															</Form.Item>
															<Form.Item
																{...field}
																...
																className="flex-1"
															>
																<Input placeholder="Value (optional)" disabled={isSubmitting} />
															</Form.Item>
															<Button
																aria-label="Remove option"
																icon={<DeleteOutlined />}
																onClick={() => remove(field.name)}
																disabled={isSubmitting || fields.length <= MIN_OPTION_COUNT}
															/>
														</Space>
													))}
													<Button
